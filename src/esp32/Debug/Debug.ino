...
const float DEBUG_TEMP_BASE_3 = 25.0;
const float DEBUG_UMID_BASE_1 = 55.0;
...
// =========================
bool alertaManualAtivo = false;
int sensorAlertaManual = 1;
float acrescimoAlertaManual = 12.0;
...
void lerSensor1(float &t, float &h) {
  if (DEBUG_SENSOR_1) {
    t = getDebugTempSensor1();
    h = getDebugHumidityOscilando(DEBUG_UMID_BASE_1, 0);
  } else {
...
void lerSensor2(float &t, float &h) {
  if (DEBUG_SENSOR_2) {
    t = DEBUG_TEMP_BASE_2;
    h = getDebugHumidityOscilando(DEBUG_UMID_BASE_2, 1);
  } else {
...
void lerSensor3(float &t, float &h) {
  if (DEBUG_SENSOR_3) {
    t = DEBUG_TEMP_BASE_3;
    h = getDebugHumidityOscilando(DEBUG_UMID_BASE_3, 2);
  } else {
...
void imprimirLeituraSensor(const char* nome, float t, float h, bool debugAtivo) {
  Serial.print(nome);
  Serial.print(" -> Temp: ");
  ...
  if (debugAtivo) Serial.println("   [DEBUG ATIVO]");
}
...
void lerComandoSerial() {
  ...
  if (comando == "alerta" || comando == "alerta1") {
    alertaManualAtivo = true;
    sensorAlertaManual = 1;
    Serial.println("SIMULACAO DE ALERTA ATIVADA NO SENSOR 1");
  } else if (comando == "alerta2") {
    alertaManualAtivo = true;
    sensorAlertaManual = 2;
    Serial.println("SIMULACAO DE ALERTA ATIVADA NO SENSOR 2");
  } else if (comando == "alerta3") {
    alertaManualAtivo = true;
    sensorAlertaManual = 3;
    Serial.println("SIMULACAO DE ALERTA ATIVADA NO SENSOR 3");
  } else if (comando == "normal") {
    alertaManualAtivo = false;
    Serial.println("SIMULACAO DESATIVADA");
  } else if (comando.startsWith("acrescimo ")) {
    String valor = comando.substring(10);
    acrescimoAlertaManual = valor.toFloat();
    Serial.print("NOVO ACRESCIMO DE ALERTA: ");
    Serial.println(acrescimoAlertaManual);
  } else {
    Serial.println("Comando desconhecido. Use: alerta, alerta1, alerta2, alerta3, normal ou 'acrescimo 12'");
  }
}

void setup() {
  Serial.begin(115200);
  Serial.println("Sistema de monitoramento com 3 DHT11");
  Serial.println("Modo debug por sensor habilitavel por true/false");
  Serial.println("Comandos seriais: alerta, alerta1, alerta2, alerta3, normal, acrescimo 12");
...
void loop() {
  ...
  if (alertaManualAtivo) {
    if (sensorAlertaManual == 1 && !isnan(t1)) t1 += acrescimoAlertaManual;
    if (sensorAlertaManual == 2 && !isnan(t2)) t2 += acrescimoAlertaManual;
    if (sensorAlertaManual == 3 && !isnan(t3)) t3 += acrescimoAlertaManual;
    Serial.println(">>> ALERTA MANUAL INJETADO VIA SERIAL <<<");
  }
  ...
  imprimirLeituraSensor("Sensor 1", t1, h1, DEBUG_SENSOR_1);
  imprimirLeituraSensor("Sensor 2", t2, h2, DEBUG_SENSOR_2);
  imprimirLeituraSensor("Sensor 3", t3, h3, DEBUG_SENSOR_3);
